...
3. **Smallest valid panel — two seats — with critique off, cheapest models, and a
   minimal diff** for any live step that is not the one 3-model smoke. Two is the floor,
   not a preference: `_MINIMUM_PANEL_REVIEWERS = 2` rejects a one-name
   `AI_REVIEW_REVIEWERS` roster, and enabling a single seat through the per-seat flags
   fails threshold clamping instead while `votes_required` is `2`. A one-reviewer panel is
   only expressible in a custom `review.yaml` with thresholds of `1`, which lives in the
   trusted image and so needs a custom image build. Treat "one seat" as unavailable.
...
|---|---|---|
| ~~Added-file lifecycle~~ — **closed at 1.0.1** | blocked at 1.0.0 by the `/dev/null` anchor defect, so every 1.0.0 fixture was modify-only. Closed as the 1.0.1 headline run: GitHub Chain B, PR #10, run `30541110970` at `5817e99`, `accepted == raw` on all three seats, inline comment `3682518404` on the added file ([record](record-github-current-image.md)); re-confirmed at `09f4e65` by the SPEC-39 Chain B run | nothing to do — **do not re-queue this**. Keep the adding fixture in later Chain B runs as cheap regression coverage, not as a gap |
| Diff **containing** a deletion — **GitHub only** | the shape the 1.0.0 `/dev/null` defect really broke: the raise fired while *scanning* the diff, before the anchor's own paths were compared, so a deleted file poisoned findings on **other** files. Every fixture since has been add/modify-only. **A GitLab run cannot close this and must not be accepted as closure:** GitLab's diff text is synthesized from API metadata as `--- a/<old>` / `+++ b/<new>`, each side falling back to the other (`gitlab_client.py:365-369`), so it never emits the `/dev/null` sentinel on either side and the defect shape cannot occur there by construction. GitHub fetches a real git diff and does. See "Added-file diffs differ by platform" above | Chain B on **GitHub**, mock-drivable today, zero tokens: fixture **deletes** one file while adding or modifying another carrying the `records[0]` marker. Assert `accepted_finding_count == raw_finding_count` **and** that the discussion posted on the marker file at the marker line — counts alone do not prove placement |
| Finding anchored **to** a deleted path | **not drivable with the deterministic mock.** Both selectors in `mock_reviewer.py` skip any line whose `kind != "added"`, and `_anchor` hard-codes `side: "new"` with `old_line: None`, so a deleting-only fixture returns an **empty batch** — which mimics the defect it was meant to test. Do not spend a campaign rediscovering this. The product path is supported (`side: "old"`; GitLab `position.old_path`/`old_line`, GitHub `LEFT`) | two routes, neither scheduled. **(1) A real two-seat run** over a diff that deletes code — the smallest panel the configuration permits, critique off, cheapest models, with one seat as the target. It must be two, not one: `_MINIMUM_PANEL_REVIEWERS = 2` rejects a one-name roster outright, and with the shipped `votes_required: 2` a single enabled seat fails threshold clamping instead, so one seat is unreachable both ways (a one-reviewer `review.yaml` with thresholds of `1` is valid but lives in the trusted image, so it needs a custom image — project-supplied policy is SPEC-47, still proposed). Two is the floor but **not automatically sufficient**, and the available paths depend on critique. **Critique off** (cheapest): `decision_for_group` surfaces inline only when `vote_count >= votes_required`, or `vote_count == 1` with `final_severity: blocker` in a `single_reviewer_blocker` category (`[security, correctness]`). **Critique on** (shipped default) adds a third path — `_recompute_group_decision` escalates a `fyi` group to `surface` when `critique_support_count > 0` and `critique.allow_advisory_escalation` is true (shipped `true`), non-blocking; on a two-seat panel that is the *easiest* route, since eligible critics exclude the group's own contributing reviewers, so the peer seat can support a finding it did not make. Critique on also risks losing the group entirely: `critique_noise_count` above half the eligible critics drops it. Anything not surfaced decides `fyi`, and `fyi_mode: summary_comment` yields a summary line with **no platform position** — that cannot close this gap. So the probe is non-deterministic twice over: the model must anchor in the removed region **and** the group must land on a surfacing path. Record which one from the consensus artifact — `decision`, `vote_count`, `contributing_reviewers`, `final_severity`, `critique_support_count` — before accepting the run. The `09f4e65` runs ran critique-off and so cannot evidence the third path: GitHub surfaced via single-reviewer-blocker (`vote_count: 1`, `['claude']`, `blocker`, support `0`), GitLab via quorum (`vote_count: 2`, `['claude','opencode']`, `major`, support `0`). **(2) Add a deletion-capable mock scenario**, the deterministic option. Either way assert the posted coordinates: GitLab `position.old_path` = deleted path, `old_line` = pre-image line, `new_line` null; GitHub `side: "LEFT"` with `line` = pre-image line |
...
| OpenCode structured-output canary at a released pin (SPEC-50) | the transport is proven against a real provider, but only on `09f4e65` images ([supplemental record](record-opencode-structured-output-canary.md)), so no record carries a `Release-*` binding. The 1.0.2 gating panel run cannot substitute: its OpenCode batch was valid but **empty**, failing `raw_finding_count > 0` | one real OpenCode review on a released pin with a non-empty batch, asserting `status: success`, `raw_finding_count > 0`, and the `used structured_output` log line. Cheapest on GitHub, whose consumer already pins the active release runtime — no repin needed. Give the fixture an obvious defect and expect to need more than one attempt; the status artifact has **no** structured-output field, so capture the job log, not just artifacts |
