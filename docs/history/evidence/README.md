...
| GitHub current image | Inline create/update, summary fallback, commands, state persistence, stale head, required blocking check | **Replacement partial** — classic-token resolve/reopen, owner-command persistence, and stale-head no-op passed; the changed-body probe fell back to summary-only behavior, and the remaining lifecycle/enforcement paths are listed in the [record](record-github-current-image.md) |
