...
| [SPEC-39](spec-39-simplification-deletion.md) | Milestone A complete; B re-scoped and ready to execute (post-1.0) | Container-only contract cleanup landed. Milestone B was re-scoped on 2026-08-12 into ordered extraction commits with frozen-contract mitigations; the three target sizes (`post.py` ~2,075, `adapter_runner.py` ~1,203, `consensus.py` ~1,044) are unchanged since the audit. Items 5 and 6 proved substantially satisfied already — workflow parity is gated three times over and the images ship no `docs/` tree — so item 5 reduces to adding the missing generation command and collapsing two of those three checks onto a shared helper, and item 6 needs nothing. The third, `scripts/check_supply_chain_pins.py`, stays standalone by image contract: it is executed from `/opt/scripts` inside the base image, so it must import only the standard library. Consensus artifact I/O is deliberately not split, since item 4's own import-coupling condition fails there. The original "sequence after SPEC-45/46" guidance is consciously waived; the resulting SPEC-53 and SPEC-45 rebases are recorded as advisory notes, not gates. |
