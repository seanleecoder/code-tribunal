...
One file, mirroring platform/github.py. GitLab was previously split across
gitlab_client.py (the API surface, in GitLab's own vocabulary of MR discussions
and notes) and a subclass here that renamed every method to the port's
vocabulary and re-wrapped every error. The rename layer is gone: the methods are
named for the port, and GitLab's terms survive only in the URLs, where they are
the API's.
"""
...
from ai_review.anchors import gitlab_line_code
...
class GitLabReviewPlatformError(ReviewPlatformError, GitLabApiError):
    """GitLab adapter error, normalized to the platform error hierarchy.

    It subclasses both so a caller may catch either the port's error type or the
    GitLab-specific one. Previously the subclass raised a sibling type, and every
    overridden method had to re-wrap; raising one type that satisfies both
    removes the wrapping without narrowing what callers can catch.
    """
...
def root_note_id_from_discussion(response: dict[str, Any]) -> int:
    ...
    if not isinstance(notes, list) or not notes:
        raise GitLabReviewPlatformError("GitLab discussion response did not include root note")
    note_id = notes[0].get("id")
    if not isinstance(note_id, int):
        raise GitLabReviewPlatformError("GitLab discussion root note did not include integer id")
    return note_id
...
class GitLabReviewPlatform:
    ...
    def _send(self, method: str, path: str, **kwargs: Any) -> Any:
        ...
        return send_with_retries(
            ...
            make_http_error=lambda status: GitLabReviewPlatformError(
                f"GitLab API {method} {path} failed: {status}"
            ),
            make_connection_error=lambda exc: GitLabReviewPlatformError(
    ...
    def _request_object(self, method: str, path: str, **kwargs: Any) -> dict[str, Any]:
        ...
        if not isinstance(parsed, dict):
            raise GitLabReviewPlatformError(
                f"GitLab API {method} {path} response was not an object"
            )
        return dict(parsed)
    ...
    def _get_all_pages(self, path: str, **kwargs: Any) -> list[dict[str, Any]]:
        ...
        for _ in range(100):
            ...
            if not isinstance(batch, list):
                raise GitLabReviewPlatformError(
                f"GitLab paginated GET {path} returned a non-list page"
            )
            for item in batch:
    ...
    def fetch_version(
        ...
        if not isinstance(versions, list) or not versions:
            raise GitLabReviewPlatformError("merge request has no versions")
    ...
    def fetch_current_head_sha(
        ...
        if not isinstance(mr, dict):
            raise GitLabReviewPlatformError("merge request response was not an object")
        head_sha = mr.get("sha") or mr.get("diff_refs", {}).get("head_sha")
        if not head_sha:
            raise GitLabReviewPlatformError(
                "merge request response did not include current head SHA"
            )
        return str(head_sha)
    ...
    def current_user(self) -> dict[str, Any]:
        ...
        if not isinstance(user, dict):
            raise GitLabReviewPlatformError("GitLab current user response was not an object")
        return user
    ...
    # --- port methods with no GitLab API call of their own -------------------
    ...
    def build_position(
        ...
        *,
        ...
        return build_position(dict(anchor), version, multiline=multiline)
