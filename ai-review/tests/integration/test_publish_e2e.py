...
class PublishEndToEndTests(unittest.TestCase):
    """End-to-end proof that the pipeline publishes and reports only publication.

    `post` is the terminal stage: its exit status is the pipeline's whole report,
    and a finding of any severity — `blocker` included — exits zero.
    """
    ...
    def test_fyi_only_consensus_posts_summary_and_exits_zero(self) -> None:
        client, consensus, post_result, exit_code = self._run_e2e(self._fyi_batches())
    ...
    ) -> tuple[FakeGitLabClient, dict[str, Any], dict[str, Any], int]:
        with tempfile.TemporaryDirectory() as tmp:
            config, manifest, diff_text = self._prepare_bundle(Path(tmp))
            client = FakeGitLabClient(head_sha=manifest["head_sha"], diff_text=diff_text)
            consensus = build_consensus(manifest, batches, config)
