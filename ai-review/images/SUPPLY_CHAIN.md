...
The pinned Cursor CLI's kernel sandbox is unavailable inside nested GitHub Actions job containers. The adapter therefore selects the CLI's native read-only Q&A mode on each print invocation with `--mode ask --sandbox disabled --trust`, avoiding a separate state-mutating setup command. Isolation for this reviewer depends on that mode, the sanitized disposable workspace, and `cli-config.json`, which allows `Read(**)` while denying relative writes with `Write(**)`, absolute writes with `Write(/**)`, and every shell command with `Shell(*)`. This is an explicit, weaker tradeoff rather than an equivalent replacement for kernel isolation. Before publishing an image from trusted `main`, `scripts/smoke_cursor_permissions.sh` first requires a successful real read probe, proving the credential and CLI path work, and then gives the same pinned CLI a hostile prompt that probes workspace creation and overwrite, redirected-home writes, and temporary-path shell effects. It distinguishes read-control failure, detected filesystem mutation, and hostile-probe execution failure while printing bounded diagnostic output. Pull requests never receive `CURSOR_API_KEY` for this smoke. A publisher without that optional secret skips the check and must keep Cursor disabled; configuring the secret turns it into a publication gate. The smoke does not claim to prove network isolation; container-level egress remains the H2 limitation. Keep Cursor disabled for consumers until the trusted-main check passes.
