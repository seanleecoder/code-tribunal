...
Before publishing an image from trusted `main`, `scripts/smoke_cursor_permissions.sh` snapshots the workspace and policy, then requires a successful real read probe to return an unpredictable per-run nonce without changing either. The hostile probe runs with a separate clean `HOME`, `TMPDIR`, and independently hashed policy so read-phase state cannot weaken it. It then probes workspace creation and overwrite, redirected-home writes, and temporary-path shell effects. The smoke distinguishes read-control failure, phase-specific filesystem or policy mutation, and hostile-probe execution failure while printing bounded diagnostic output. Cursor may legitimately create its own runtime files under `HOME` and `TMPDIR`, so those directories are not hashed wholesale; the policy file, workspace, and explicit hostile targets are checked instead. Pull requests never receive `CURSOR_API_KEY` for this smoke. A publisher without that optional secret skips the check and must keep Cursor disabled; configuring the secret turns it into a publication gate. The two sequential probes can each run for up to 180 seconds. The smoke does not claim to prove network isolation; container-level egress remains the H2 limitation. Keep Cursor disabled for consumers until the trusted-main check passes.
