...
| `AI_REVIEW_GITHUB_RESOLVE_TOKEN` | GitHub review-thread mutations | Optional dedicated Actions secret used only for GraphQL resolve/unresolve mutations. Use a repository-scoped maintainer token with Pull requests read/write access when GitHub rejects the built-in `GITHUB_TOKEN`; ordinary comments and state continue using `GITHUB_TOKEN`. |
